...
function go() {
    let prx = '1.1.1.1:3128';

    if(customIP) {
        prx = customIP.split(':');
    } else {
        prx = proxy[~~(Math.random() * proxy.length)].split(':');
    }

    let tlsSocket;

    if (!prx[1] || isNaN(prx[1])) {
        go()
        return
    }

    const netSocket = net.connect(Number(prx[1]), prx[0], () => {
        netSocket.once('data', () => {
        ...
        if (prx[2] && prx[3]) {
        netSocket.write(`CONNECT ${url.host}:443 HTTP/1.1\r\n` + `Host: ${url.host}:443\r\n` + `Proxy-Authorization: Basic ${Buffer.from(`${prx[2]}:${prx[3]}`).toString('base64')}\r\n` + `Proxy-Connection: Keep-Alive\r\n\r\n`);
         } else {
        netSocket.write(`CONNECT ${url.host}:443 HTTP/1.1\r\nHost: ${url.host}:443\r\nProxy-Connection: Keep-Alive\r\n\r\n`)
        }
